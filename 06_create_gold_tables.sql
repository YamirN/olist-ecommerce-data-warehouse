...
CREATE TABLE gold.dim_date (
    date_key INT PRIMARY KEY,            -- YYYYMMDD (ej: 20180101)
    date DATE NOT NULL,
    year INT,
    quarter INT,
    month INT,
    ...
    is_weekend BIT,
    is_holiday BIT DEFAULT 0             -- Para cargar feriados después
);
GO
...
CREATE TABLE gold.dim_customer (
    ...
    customer_id NVARCHAR(100) NOT NULL,         -- Business Key (Olist ID)
    customer_unique_id NVARCHAR(100),           -- ID único real del cliente
    customer_city NVARCHAR(100),
    customer_state NVARCHAR(10),
    geolocation_lat DECIMAL(10,6),              -- Traído de geolocation (si aplica)
    geolocation_lng DECIMAL(10,6),
    effective_start_date DATETIME2(3) DEFAULT SYSDATETIME(), -- Para SCD Tipo 2 (futuro)
...
-- 1.3 DIM_PRODUCT
-- Combina Products + Category Translation
IF OBJECT_ID('gold.dim_product', 'U') IS NOT NULL DROP TABLE gold.dim_product;
CREATE TABLE gold.dim_product (
    product_sk INT IDENTITY(1,1) PRIMARY KEY,
    product_id NVARCHAR(100) NOT NULL,
    category_name NVARCHAR(100),           -- Nombre original (PT)
    category_name_english NVARCHAR(100),   -- Nombre inglés (EN)
...
CREATE TABLE gold.fact_orders (
    ...
    order_id NVARCHAR(100) NOT NULL,       -- Degenerate Dimension (útil para drill-through)
